...
# 주의: 아래 계산은 기본요금 + 전력량요금만 반영한 단순 모델이다.

# 실제 청구금액에는 부가가치세, 전력산업기반기금, 기후환경요금, 연료비조정요금 등이 추가될 수 있다.
...
KPX_SMP_2025_LAND["annual_average"] = sum(KPX_SMP_2025_LAND["monthly"].values()) / 12
...
# 한국에너지공단 주택지원사업 보조금 예시값

# 주의: 총 설치비가 아니라 보조금 단가 예시로만 사용한다.
...
epw_path = os.path.join(os.path.dirname(**file**), file_name)
